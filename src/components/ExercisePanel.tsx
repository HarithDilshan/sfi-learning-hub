...
export default function ExercisePanel({ exercises, topicId }: Props) {
  ...
  function handleFill() {
    ...
    setAnswered(true);
    const correct = fillValue.trim().toLowerCase() === (ex.answer as string).toLowerCase();
    setIsCorrect(correct);
    if (correct) setScore((s) => s + 1);
    setTimeout(advance, 1500);
  }
  ...
  function advance() {
    if (currentQ + 1 >= exercises.length) {
      ...
      const pct = Math.round(((score + (isCorrect ? 0 : 0)) / exercises.length) * 100);
      // Use current score state which may not be updated yet, so recalculate
      setFinished(true);
      return;
    }
    setCurrentQ((q) => q + 1);
  ...
  if (finished) {
    ...
    return (
      <div className="text-center py-10 animate-slide-up">
        ...
        >
          {pct}%
          <span className="text-xs font-medium">{score}/{exercises.length}</span>
        </div>
        <h3 className="text-xl font-semibold mb-2">{msg}</h3>
        <p className="mb-6" style={{ color: "var(--text-light)" }}>You earned {score * 10} XP from this exercise!</p>
        <div className="flex gap-3 justify-center">
          <button onClick={restart} className="px-7 py-3 rounded-lg font-semibold text-white cursor-pointer" style={{ background: "var(--blue)" }}>
            Try Again
          </button>
        </div>
      </div>
  ...
  return (
    <div>
      <p className="mb-2" style={{ color: "var(--text-light)", fontSize: "0.95rem" }}>
      ...
      <div className="w-full h-2 rounded overflow-hidden mb-6" style={{ background: "var(--warm-dark)" }}>
        <div className="h-full rounded progress-fill-animate" style={{ width: `${progress}%`, background: "linear-gradient(90deg, var(--blue), var(--forest))" }} />
      </div>
      ...
      <div className="rounded-xl p-8" style={{ background: "var(--warm)" }}>
        ...
        >
          ...
          {ex.type === "mc" && ex.options && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {ex.options.map((opt, i) => {
                ...
                if (answered) {
                  if (i === ex.answer) btnStyle = "border-[var(--correct)] bg-[var(--correct-bg)]";
                  else if (i === selectedOption) btnStyle = "border-[var(--wrong)] bg-[var(--wrong-bg)]";
          ...
          {ex.type === "fill" && (
            <>
              {ex.hint && (
                <div className="text-sm mb-3" style={{ color: "var(--text-light)" }}>Hint: {ex.hint}</div>
              )}
          ...
          {answered && (
            <div className={`flex items-center gap-2 px-4 py-3 rounded-lg mt-3 text-sm font-medium animate-fade-in ${isCorrect ? "bg-[var(--correct-bg)] text-[var(--correct)]" : "bg-[var(--wrong-bg)] text-[var(--wrong)]"}`}>
              {isCorrect ? "✅ Rätt! (Correct!)" : `❌ Fel — the answer is: ${ex.type === "mc" ? ex.options?.[ex.answer as number] : ex.answer}`}
            </div>
          )}
        </div>
      </div>
